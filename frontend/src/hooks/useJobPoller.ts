...
// import { getStatus } from '@/lib/api'; // TODO: implement in backend
...
export function useJobPoller() {
  ...
  const startPolling = () => {
    ...
    intervalRef.current = setInterval(async () => {
      try {
        // const s = await getStatus(); // TODO: implement in backend
        // setState(s);
  ...
  useEffect(() => {
    // getStatus().then(setState).catch(() => {}); // TODO: implement in backend
    return stopPolling;
  }, []);
